...
@application.before_first_request #For when application starts load model
def startup():
...
@application.route("/", methods=['POST', 'GET']) #once model loads build the page
def build_page():
        #First check if user is asking/requesting for something
        if request.method == 'POST': 
            ...
            return render_template('index.html',
                date_horizon = horizon_date.strftime('%m/%d/%Y'),
                earthquake_horizon = cloudword_generator(str(horizon_date)[:10], earthquake_live),
                current_value=horizon_int, 
                days_out_to_predict=days_out_to_predict)
